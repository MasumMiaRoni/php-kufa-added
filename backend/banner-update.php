...
$description = $_POST['description'];
	...
	if ($_FILES['images']['name']) {
		...
		if ($allow) {
			if ($_FILES['images']['size'] < 50000000){
				...
				$update = "UPDATE banner SET images = '$img_id'  title = '$title' ,description = '$description' WHERE id = '$id'";		
	...
	$update = "UPDATE banner SET title = '$title' ,description = '$description' WHERE id = '$id'";	
	$update_query = mysqli_query($db,$update);	
